...
export interface InspectorContext {
  activeElement: ElementSelectInfo | ElementHoverInfo;
  activeTab: TabInfo | null;
  selectedElement: ElementSelectInfo | null;
  hoveredElement: ElementHoverInfo | null;
  inspectedElements: ElementSelectInfo[];
  setInspectedElements: React.Dispatch<React.SetStateAction<ElementSelectInfo[]>>;
  showSvgMarkup: boolean;
  setShowSvgMarkup: (show: boolean) => void;
  tokenSystem: 'semantic' | 'tailwind' | 'material';
  setTokenSystem: (sys: 'semantic' | 'tailwind' | 'material') => void;
  addDevLog: (direction: 'in' | 'out' | 'system', type: string, message: string) => void;
  spacingHistory: number[];
  radiusHistory: string[];
  card17Tab: 'summary' | 'export';
  setCard17Tab: (tab: 'summary' | 'export') => void;
  downloadTokenFile: (filename: string, content: string) => void;
  
  // Statistical aggregators for Card 17
  getPrimaryFont: () => { fontName: string; source: string; isGoogleFont: boolean; googleFontUrl?: string };
  getTopColors: () => Array<{ hex: string; rgb: string; count: number; token: string; role: string; compliance: string; contrast: string }>;
  getTopSpacing: () => { scale: Array<{ valuePx: number; tokenName: string; count: number }>; consistency: number; compliance8pt: number; is8ptGrid: string };
  getTopRadii: () => { scale: Array<{ value: string; tokenName: string; count: number }>; consistency: number };
  getTopShadows: () => { scale: Array<{ raw: string; classification: string; count: number }>; consistency: number; avgElevation: string; glassCount: number };
}
...
export const moduleRegistry: InspectorModule[] = [
  // 1. Typography
  {
    id: 'typography',
    title: 'Typography',
    supportedCategories: ['Text', 'Heading', 'Paragraph', 'Link', 'Button', 'Input', 'Textarea'],
    order: 10,
    render: (ctx) => {
      const activeElement = ctx.activeElement;
      const selectedElement = ctx.selectedElement;
  ...
  // 2. Colors
  {
    id: 'colors',
    title: 'Colors',
    supportedCategories: ['Text', 'Heading', 'Paragraph', 'Link', 'Button', 'Input', 'Textarea', 'Image', 'Picture', 'SVG', 'Video', 'Canvas', 'Iframe', 'Section', 'Container', 'Navigation', 'List', 'Card', 'Background Image', 'Unknown'],
    order: 20,
    render: (ctx) => {
      const activeElement = ctx.activeElement;
      return (
    ...
    render: (ctx) => {
      ...
      return (
        <InspectorCard
          ...
          isEmpty={!activeElement || !activeElement.layout}
        >
  ...
  // 5. Assets
  {
    id: 'assets',
    title: 'Assets',
    supportedCategories: ['Image', 'Picture', 'SVG', 'Video', 'Canvas', 'Background Image'],
    order: 50,
    render: (ctx) => {
      const activeElement = ctx.activeElement;
      const showSvgMarkup = ctx.showSvgMarkup;
      const setShowSvgMarkup = ctx.setShowSvgMarkup;
      const addDevLog = ctx.addDevLog;

      if (!activeElement || !activeElement.asset) {
        return (
          <InspectorCard
            key="assets"
            title="Assets"
              ...
              {(!hasSvg || !showSvgMarkup) ? (
                <div className="space-y-2 text-[10px] font-mono">
                  ...
                  <div className="flex items-center justify-between text-zinc-500">
                    ...
                    <div className="flex items-center gap-1.5">
                      <div className="w-16 h-1.5 bg-zinc-950 border border-zinc-900 rounded-full overflow-hidden">
                        <div 
  ...
  // 6. Background
  {
    id: 'background',
    title: 'Background',
    supportedCategories: ['Container', 'Background Image', 'Button', 'Input', 'Text', 'Link', 'Section', 'Navigation', 'List', 'Card', 'Unknown'],
    order: 60,
    render: (ctx) => {
      const activeElement = ctx.activeElement;
      if (!activeElement || !activeElement.background) {
        return (
          <InspectorCard
            key="background"
            title="Background"
  ...
  // 7. Shadows
  {
    id: 'shadows',
    title: 'Shadows',
    supportedCategories: ['Text', 'Heading', 'Paragraph', 'Link', 'Button', 'Input', 'Textarea', 'Image', 'Picture', 'Iframe', 'Section', 'Container', 'Navigation', 'List', 'Card', 'Background Image', 'Unknown'],
    order: 70,
    render: (ctx) => {
      const activeElement = ctx.activeElement;
      const effects = activeElement?.effects;
      const boxShadows = effects?.boxShadows || [];
      const dropShadows = effects?.dropShadows || [];
      ...
      return (
        <InspectorCard
          ...
          isEmpty={!activeElement || !effects || allShadows.length === 0}
        >
          {effects && (
            <div className="space-y-3">
  ...
  // 8. Filters & Blending
  {
    ...
    render: (ctx) => {
      ...
      const isCardEmpty = !activeElement || !eff || (
        (eff.filter === 'none' || !eff.filter) &&
        (eff.backdropFilter === 'none' || !eff.backdropFilter) &&
        (eff.opacity === '1' || !eff.opacity) &&
        (eff.mixBlendMode === 'normal' || !eff.mixBlendMode) &&
        (eff.isolation === 'auto' || !eff.isolation)
      );
  ...
  // 9. Border Radius
  {
    id: 'borderRadius',
    title: 'Border Radius',
    supportedCategories: ['Text', 'Heading', 'Paragraph', 'Link', 'Button', 'Input', 'Textarea', 'Image', 'Picture', 'Video', 'Canvas', 'Iframe', 'Section', 'Container', 'Navigation', 'List', 'Card', 'Background Image', 'Unknown'],
    order: 90,
    render: (ctx) => {
      const activeElement = ctx.activeElement;
      const br = activeElement?.effects?.borderRadius;
      const isCardEmpty = !activeElement || !br || (
        br.topLeft === '0px' &&
        br.topRight === '0px' &&
        br.bottomRight === '0px' &&
        br.bottomLeft === '0px'
  ...
  // 11. Design Tokens
  {
    ...
    render: (ctx) => {
      const activeElement = ctx.activeElement;
      const tokenSystem = ctx.tokenSystem;
      const setTokenSystem = ctx.setTokenSystem;
    ...
    render: (ctx) => {
      ...
      return (
        <InspectorCard
          ...
          emptyMessage="Computing typography intelligence..."
          isEmpty={false}
        >
          <div className="space-y-3.5 font-mono text-[10px]">
    ...
    render: (ctx) => {
      ...
      return (
        <InspectorCard
          ...
          emptyMessage="Computing color intelligence..."
          isEmpty={false}
        >
  ...
  // 14. Spacing Intelligence
  {
    id: 'spacingIntelligence',
    ...
    render: (ctx) => {
      ...
      const spacingHistory = ctx.spacingHistory;

      if (!activeElement || !si) {
        return (
          <InspectorCard
            key="spacingIntelligence"
            title="Spacing Intelligence"
            icon={
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
      ...
      return (
        <InspectorCard
          ...
          emptyMessage="Computing spacing intelligence..."
          isEmpty={false}
        >
          <div className="space-y-4 font-mono text-[10px]">
  ...
  // 15. Border Radius Intelligence
  {
    id: 'borderRadiusIntelligence',
    ...
    render: (ctx) => {
      ...
      const radiusHistory = ctx.radiusHistory;

      if (!activeElement || !bri) {
        return (
          <InspectorCard
            key="borderRadiusIntelligence"
            title="Border Radius Intelligence"
            icon={
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
      ...
      return (
        <InspectorCard
          ...
          emptyMessage="Computing border-radius intelligence..."
          isEmpty={false}
        >
          <div className="space-y-3.5 font-mono text-[10px]">
            <div className="space-y-2">
    ...
    render: (ctx) => {
      ...
      return (
        <InspectorCard
          ...
          emptyMessage="Computing shadow intelligence..."
          isEmpty={false}
        >
          <div className="space-y-3.5 font-mono text-[10px]">
            <div className="space-y-2">
            ...
            <p className="text-[8.5px] text-zinc-500 leading-relaxed pl-1.5 border-l border-zinc-800 italic pt-1">
              {si.classification === 'Glass Effect' ? 'Glassmorphic card detected. Uses soft backdrop blurring and layered opacity.' :
               si.classification === 'None' ? 'Flat design style with zero shadow projection. Ideal for nested elements or flat shapes.' :
  ...
  // 17. Design System Summary
  {
    id: 'designSystemSummary',
    title: 'Design System Summary',
    supportedCategories: ['Text', 'Heading', 'Paragraph', 'Link', 'Button', 'Input', 'Textarea', 'Image', 'Picture', 'SVG', 'Video', 'Canvas', 'Iframe', 'Section', 'Container', 'Navigation', 'List', 'Card', 'Background Image', 'Unknown'],
    order: 170,
    render: (ctx) => {
      const inspectedElements = ctx.inspectedElements;
      const setInspectedElements = ctx.setInspectedElements;
      const card17Tab = ctx.card17Tab;
      const setCard17Tab = ctx.setCard17Tab;
      const downloadTokenFile = ctx.downloadTokenFile;
      const addDevLog = ctx.addDevLog;

      const font = ctx.getPrimaryFont();
      const colors = ctx.getTopColors();
      const spacing = ctx.getTopSpacing();
      const radii = ctx.getTopRadii();
      const shadows = ctx.getTopShadows();

      return (
        <InspectorCard
          key="designSystemSummary"
          title="Design System Summary"
          icon={
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" />
          ...
          isEmpty={inspectedElements.length === 0}
        >
          <div className="space-y-4 font-mono text-[10px]">
...
// Register all modules with the registry singleton
moduleRegistry.forEach(mod => moduleRegistryInstance.registerModule(mod));
